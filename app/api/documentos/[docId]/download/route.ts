  ...
  try {
    ...
    if (!url || !serviceKey) {
      return NextResponse.json(
        { ok: false, error: 'Variables de entorno faltantes (URL o SERVICE KEY)' },
        { status: 500 }
      )
    }
    ...
    // 2) Generar URL firmada (60 minutos)
    ...
    if (signErr) {
      return NextResponse.json({ ok: false, step: 'sign', error: signErr.message }, { status: 500 })
    }
    if (!signed?.signedUrl) {
